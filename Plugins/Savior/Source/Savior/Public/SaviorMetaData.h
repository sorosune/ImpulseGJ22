...
class SAVIOR_API UMetaSlot : public USaveGame {
	GENERATED_BODY()
public:
...
class SAVIOR_API UDataSlot : public USaveGame {
	GENERATED_BODY()
public:
...
struct SAVIOR_API FSlotData {
	...
	{///
		if (DS->IsValidLowLevelFast()) {
			Minimal.Append(DS->Minimal);
			Complex.Append(DS->Complex);
		}
	}
	//
	//
	inline FSlotData & operator = (const FSlotData &SD) {
		...
		//Complex = SD.Complex;
		//
		Minimal.Append(SD.Minimal);
		Complex.Append(SD.Complex);
	return *this;}
	//
