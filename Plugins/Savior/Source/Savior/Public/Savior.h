...
class SAVIOR_API USaviorSettings : public UObject {
	GENERATED_BODY()
	//
	USaviorSettings();
public:
...
class SAVIOR_API UAutoInstanced : public UObject, public ISAVIOR_Serializable {
	GENERATED_BODY()
};

UCLASS(ClassGroup=Synaptech, Category="Synaptech", BlueprintType, meta=(DisplayName="Savior"))
class SAVIOR_API USavior : public UObject, public Reflector {
	GENERATED_BODY()
	//
...
class SAVIOR_API USaviorSubsystem : public UGameInstanceSubsystem {
	GENERATED_BODY()
public:
	UPROPERTY() USavior* StaticSlot;
public:
...
SAVIOR_API bool ParseActorTAG(const AActor* Actor, const FName Tag);

/** Return True of a Component hosts an specific Tag in its Tag List. */
SAVIOR_API bool ParseComponentTAG(const UActorComponent* Component, const FName Tag);
...
static void OnPreparedToSave() {
	for (TObjectIterator<UObject>OBJ; OBJ; ++OBJ) {
		...
		if (OBJ->IsA(USavior::StaticClass())) {continue;}
...
static void DeserializeGameWorld(USavior* Savior) {
	...
	if (UGameInstance*const&GI=Savior->GetWorld()->GetGameInstance()) {
		System = GI->GetSubsystem<USaviorSubsystem>();
	} if (System) {USavior::ShadowCopySlot(Savior,System->StaticSlot,Result);}
	//
...
static void GT_NewObjectInstance(USavior* Savior, FObjectProperty* Property, UObject* Outer, UObject* CDO, const FString FullName) {
...
static void GT_NewObjectIntoArray(USavior* Savior, FArrayProperty* Property, UObject* Outer, UObject* CDO, const FString FullName, const int32 Index) {
	if (!IsInGameThread()||(Outer==nullptr)||(CDO==nullptr)||(Property==nullptr)) {return;}
	//
	...
	//
	//
	FString Name, Num, Temp, SequenceNum;
	FString TempName = FullName;
	FString Remove = "_" + SGUID;
	TempName = TempName.Replace(*Remove, TEXT(""));
	TempName.Split("_", nullptr, &Num, ESearchCase::IgnoreCase, ESearchDir::FromEnd);
	if(Num.IsNumeric())
		Name = (SGUID.IsEmpty()) ? CDO->GetName() : (CDO->GetName() + TEXT("_") + Num).Replace(TEXT("Default__"), TEXT(""));
	else
		Name = (SGUID.IsEmpty()) ? CDO->GetName() : (CDO->GetName() + TEXT("_") + FString::FromInt(Index)).Replace(TEXT("Default__"), TEXT(""));

	if (Name.Contains(TEXT("_C_"))) {
		Temp = Name;
		Name.Split(TEXT("_C_"), &Name, &SequenceNum, ESearchCase::CaseSensitive, ESearchDir::FromEnd);
		...
		Name.Split(TEXT("_C"), &Name, nullptr, ESearchCase::CaseSensitive, ESearchDir::FromEnd);
...
static void GT_NewObjectIntoSet(USavior* Savior, FSetProperty* Property, UObject* Outer, UObject* CDO, const FString FullName) {
	if (!IsInGameThread()||(Outer==nullptr)||(CDO==nullptr)||(Property==nullptr)) {return;}
	//
...
static void GT_NewObjectIntoMapKey(USavior* Savior, FMapProperty* Property, UObject* Outer, UObject* CDO, const FString FullName) {
	if (!IsInGameThread()||(Outer==nullptr)||(CDO==nullptr)||(Property==nullptr)) {return;}
	//
...
static void GT_NewObjectIntoMapValue(USavior* Savior, FMapProperty* Property, UObject* Outer, UObject* Key, UObject* CDO, const FString FullName) {
	if (!IsInGameThread()||(Outer==nullptr)||(CDO==nullptr)||(Property==nullptr)) {return;}
	//
...
static void GT_NewObjectIntoNamedMapValue(USavior* Savior, FMapProperty* Property, UObject* Outer, FName Key, UObject* CDO, const FString FullName) {
	if (!IsInGameThread()||(Outer==nullptr)||(CDO==nullptr)||(Property==nullptr)) {return;}
	//
...
private:
	USavior* Savior;
public:
	TASK_SerializeGameWorld(USavior* Savior) {this->Savior=Savior;}
	//
...
private:
	USavior* Savior;
public:
	TASK_DeserializeGameWorld(USavior* Savior) {this->Savior=Savior;}
	//
	FORCEINLINE TStatId GetStatId() const {
...
private:
	USavior* Savior;
public:
	TASK_SerializeGameInstance(USavior* Savior) {this->Savior=Savior;}
	//
	FORCEINLINE TStatId GetStatId() const {
...
private:
	USavior* Savior;
public:
	TASK_DeserializeGameInstance(USavior* Savior) {this->Savior=Savior;}
	//
	FORCEINLINE TStatId GetStatId() const {
